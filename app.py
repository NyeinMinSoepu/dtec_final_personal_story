...
with tab1:
    ...
    st.write("My service learning internship journey begins at the End of July 2025 and lasts till the end of the year.")
    st.write("During those times, I have been abled to work on different aspects such as learning new skills by attending new courses, improving academic writing, and research paper developments.")
    st.write("This is mainly due to the the opportunities given by Parami, and the mentors and people that I have met there.")
    st.write("Please, take a look at what I have done to understand my journey through this learning.")
    ...
    ax1.set_title("The Time Investment (Total Hours)", fontsize=14, fontweight='bold', pad=20)
...
with tab2:
    if time_granularity == "Days":
        ...
        st.subheader("Weekly Productivity, Stress and Enjoyment Over Weeks")
        st.write("The pictures below shows the sum for each week of stress, daily working time, and enjoyment over the period of six months.")
        
        weekly = df.groupby('week', sort=True).agg({'duration': 'sum', 'personal_enjoyment': 'mean', 'stress_level': 'mean'}).reset_index()
        
        fig3, (ax_w1, ax_w2) = plt.subplots(2, 1, figsize=(12, 8), sharex=True)
        
        ax_w1.plot(weekly['week'], weekly['duration'], color='#3498db', marker='s', linewidth=2, label='Total Weekly Hours')
        ax_w1.fill_between(weekly['week'], weekly['duration'], color='#3498db', alpha=0.1)
        ax_w1.set_title("The Chronological Journey: Weekly Workload & Well-being Trends", fontsize=14, fontweight='bold', pad=10)
        ax_w1.set_ylabel("Hours Worked / Week", fontsize=11)
        ax_w1.grid(True, linestyle='--', alpha=0.5)
        
        ax_w2.plot(weekly['week'], weekly['personal_enjoyment'], color='#2ecc71', marker='s', linewidth=2, label='Average Enjoyment')
        ax_w2.plot(weekly['week'], weekly['stress_level'], color='#e74c3c', marker='d', linestyle='--', linewidth=2, label='Average Stress')
        ax_w2.set_ylabel("Metric Score (1 - 10)", fontsize=11)
        ax_w2.set_xlabel("Week Start Date", fontsize=11)
        ax_w2.set_ylim(1, 10)
        ax_w2.grid(True, linestyle='--', alpha=0.5)
        ax_w2.legend(loc='upper left', frameon=True, facecolor='white')
        ...
        st.markdown("""
        Following is the weekly trend breakdown for aggregated internship productivity:
        - View shifts show smoothed trend variations over the 6-month lifecycle.
        - Keeps context cleaner than high-variance daily data points.
        """)
